...
public class GeoDatabaseHelper implements MobileNetworkDataCapable, SQLExecutable {
    ...
    private final String positionsExistsQuery =
    ...
    public synchronized void deleteFrom(String table) {
        String statement = "DELETE FROM " + table +";";
        try {
            mDb.compile(statement);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean insertRecord(Call call) {
        ...
        // Notify listeners
        for (CDRDatabaseInsertionListener l : listeners) {
    ...
    public void insertRecord(Position position) {
        String insertPositionStatement = "INSERT INTO Positions (lat, lng, acc, cell_id, Geometry)" +
                " SELECT %1$s, %2$s, %3$s, %4$s, %5$s" + " WHERE NOT EXISTS (" + positionsExistsQuery + " );";
        execSQL(String.format(insertPositionStatement, String.valueOf(position.getLat()), String.valueOf(position.getLng()),
                String.valueOf(position.getAccuracy()), String.valueOf(position.getCellId()), String.format("GeomFromText('POINT(%s %s)', 4326)", String.valueOf(position.getLng()), String.valueOf(position.getLat()))));
    }
    ...
    public boolean insertMeasurements(CellInfo cellInfo, final int eventId, final int eventType) {
        ...
        // Maybe the Position Futures are being frozen when they actually should still be running
    ...
    public Position getPositionsFromId(int id) {
        Position position = null;
        String selectPosition = "SELECT id, lat, lng, acc, cell_id " +
                "  FROM Positions WHERE cell_id = %1$s;";
        try {
            TableResult tableResult = mDb.get_table(String.format(selectPosition, String.valueOf(id)));
            if (!tableResult.rows.isEmpty()) {
                String[] row = ((String[]) tableResult.rows.get(0));
    ...
    public ArrayList<Position> getAllPositionRecords() {
        ArrayList<Position> locationsArrayList = new ArrayList<Position>();
        final String selectAllPositions =
                "SELECT id, lat, lng, acc, cell_id " +
                        "   FROM Positions;";
        try {
            TableResult tableResult = mDb.get_table(selectAllPositions);
            Vector<String[]> rows = tableResult.rows;
            for (String[] fields : rows) {
                Position position = parsePosition(fields);
                locationsArrayList.add(position);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private Position parsePosition(String[] fields) {
        double lat = Double.valueOf(fields[1]);
        double lng = Double.valueOf(fields[2]);
        double acc = Double.valueOf(fields[3]);
        int cellId = Integer.valueOf(fields[4]);
        return new Position(lat, lng, acc, cellId);
    }
    ...
    public CellInfo getCellById(long id) {
        ...
        try {
            ...
            return new CellInfo(Integer.parseInt(fields[0]), Integer.parseInt(fields[1]), Integer.parseInt(fields[2]),
    ...
    public ArrayList<CellInfo> getAllCellRecords() {
        ...
        final String selectAllCells =
                "SELECT cellid, lac, mnc, mcc, technology, id" +
                        "   FROM Cells;";
        try {
    ...
    private CellInfo parseCell(String[] fields) {
        ...
        int id = Integer.valueOf(fields[5]);
